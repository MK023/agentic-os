...
## Execution log — 2026-07-28 (evening)

Marco said go. All eight blocks are **written, executed and verified locally**;
the four boxes still open below are the ones that need accounts or live infra,
nothing else. Branch: `feat/phase1-observability-hub` (this repo) and
`feat/agentic-os-status-widget` (marcobellingeri.dev). Not pushed.

What verification found — bugs the plan carried, each fixed in the code, none
of them findable by re-reading the plan:

| # | Bug in the plan | Effect if run as written |
|---|---|---|
| 1 | `logs` pipeline exports to `prometheus` | Collector refuses to start (that exporter is metrics-only) |
| 2 | Metric names assumed `claude_code_*_total` | Every panel and query empty — the exporter's default strategy appends type+unit suffixes; now pinned with `translation_strategy` |
| 3 | `OTEL_METRICS_EXPORTER_OTLP_TEMPORALITY_PREFERENCE` | Variable does not exist; silently ignored, leaving delta where Prometheus needs cumulative |
| 4 | `file(var.ssh_public_key_path)` with a `~` default | `terraform plan` fails — `file()` does not expand `~` |
| 5 | Task 1 validates before Task 4 writes `bootstrap.sh` | `terraform validate` fails on a missing file: task order was wrong |
| 6 | `HEALTHCHECK` uses `curl` | `python:3.12-slim` ships no curl — every healthcheck fails. Now stdlib `urllib` |
| 7 | Widget sends only `CF-Access-*` headers | Status API has its own token too: always 401, widget permanently degraded |
| 8 | Site route not in `run_worker_first` | `/api/agentic-status` served by the Asset Worker: 404 |
| 9 | Widget fetched in the Astro frontmatter | Static build: numbers frozen at deploy time, not live |
| 10 | `.checkov.yml` created, no job runs it | A gate that does not exist |
| 11 | Cloudflare doc mixed locally- and remotely-managed tunnels | `tunnel create` never prints the `TUNNEL_TOKEN` the compose file needs |
...
Image tags and Python dependencies were repinned to current stable: the planned
ones were from 2024–2025.

Verified by running, not reading: `terraform init/fmt -check/validate` clean ·
`docker compose config --quiet` exit 0 · 8 pytest tests, 100% coverage ·
6 new `node:test` tests in the site repo, 112 green overall there, `lint` and
`check` clean · zizmor clean at HIGH · checkov 156 passed / 0 failed ·
`verify-hub.sh` fails as documented · mutmut: 0 survivors in the auth function.

**Docker itself was still not run** (no daemon on this machine): image pulls,
`docker build`, and the Collector actually starting with this config remain
unproven until the VPS exists.
...
- [x] Cloudflare account with a zone + `CLOUDFLARE_API_TOKEN` (Tunnel edit) — **already have this**
- [x] SSH keypair to attach to the VPS
- [x] `gh` / git access to push this repo and to `MK023/marcobellingeri.dev` (Block 7 touches that repo)
...
Checklist:
- [x] Write `versions.tf` (Task 1, Step 1) — provider block, `HOSTINGER_API_TOKEN` from env, never hardcoded
- [x] Write `variables.tf` (Task 1, Step 2) — `vps_plan`, `data_center_id`, `template_id`, `ssh_public_key_path`
- [x] Write `main.tf` (Task 1, Step 3) — `hostinger_vps_ssh_key`, `hostinger_vps_post_install_script`, `hostinger_vps` resources
- [x] Write `outputs.tf` (Task 1, Step 4) — `vps_ip` output
- [x] Write `terraform.tfvars.example` (Task 1, Step 5) with placeholder IDs marked REPLACE
...
- [x] `terraform fmt -check && terraform validate` — must pass before commit (Task 1, Step 7)
- [x] Commit (Task 1, Step 8)
...
Checklist:
- [x] Write `otel-collector-config.yaml` (Task 2, Step 1) — OTLP receiver **with `bearertokenauth` extension**. This is the fix for the CVE-2026-28798-class gap found in the security sweep: an OTLP ingest endpoint behind a public Cloudflare Tunnel hostname must not be left unauthenticated.
- [x] Write `prometheus.yml` (Task 2, Step 2) — scrape config, 15s interval
- [x] Write Grafana provisioning: datasource (Step 3), dashboard provider (Step 4), dashboard JSON (Step 5) — **verify the `claude_code_*` metric names against your installed Claude Code version before relying on this dashboard** (they're versioned and can change)
- [x] Write `docker-compose.yml` (Task 2, Step 6) — 4 services (`cloudflared`, `otel-collector`, `prometheus`, `grafana`) + `status-api` (Block 3). **Every image pinned to a version, never `:latest`** — check each image's current stable tag before running (the versions in the plan were confirmed to exist on 2026-07-28, but this file may run months later). Every service gets `security_opt: [no-new-privileges:true]` + `cap_drop: [ALL]`.
- [x] **Gate**: `docker compose config --quiet` with dummy env values — must exit 0 (Task 2, Step 7)
- [x] Commit (Task 2, Step 8)
...
Checklist:
- [x] Write `test_main.py` — **5 tests**, not 4 (Task 3, Step 1): whitelisted fields, missing token 401, wrong token 401, upstream HTTP failure → 502 + Sentry capture, **malformed-JSON upstream response → 502 + Sentry capture** (this 5th test was added 2026-07-28 after actually running the suite exposed a real gap — see below)
- [x] Write `conftest.py` (Task 3, Step 2) — sets `PROMETHEUS_URL`/`STATUS_API_TOKEN` env vars before `main.py` is imported by the test module
- [x] Run tests, confirm they fail (`ModuleNotFoundError`) (Task 3, Step 3)
- [x] Write `requirements.txt` (Task 3, Step 4) (fastapi, uvicorn, httpx — 3 deps, deliberately minimal)
- [x] Write `sentry.py` (Task 3, Step 5) — zero-dependency envelope client, ported from `marcobellingeri.dev/engine/lib/sentry.mjs`, same fail-open contract
- [x] Write `main.py` (Task 3, Step 6) — **four things fixed/added on 2026-07-28, read the plan's note in full before writing this file:**
...
Checklist:
- [x] Write `scripts/bootstrap.sh` (Task 4, Step 1) — clones this repo onto the VPS, refuses to start the stack without a real `docker/.env`
- [x] Write `docker/.env.example` (Task 4, Step 2) listing every required var, no real values — now 5 vars: `CLOUDFLARE_TUNNEL_TOKEN`, `GRAFANA_ADMIN_PASSWORD`, `STATUS_API_TOKEN`, `OTLP_INGEST_TOKEN`, `SENTRY_DSN`
- [x] Add `docker/.env` and `terraform/hostinger-vps/terraform.tfvars` to `.gitignore` (Task 4, Step 3) — **real secrets never committed**
- [x] Commit (Task 4, Step 4)
...
Checklist:
- [x] Write `docs/CLOUDFLARE_TUNNEL_SETUP.md` (Task 5, Step 1)
- [ ] Commit the doc now; **execute** the manual sequence inside it only once Blocks 1-4 are deployed for real (Task 5, Step 2)
- [x] Three hostnames: Grafana (Access, your email only), status API (Access, Service Auth token — feeds Block 7), OTel ingest (**no Access app** — Claude Code can't do the service-token header — auth is the `bearertokenauth` token from Block 2 instead, not "left open because the hostname is hard to guess")
...
Checklist:
- [x] Write `docs/CLAUDE_CODE_TELEMETRY.md` (Task 6, Step 1) — includes `OTEL_EXPORTER_OTLP_HEADERS=Authorization=Bearer <token>`, the header the Collector's `bearertokenauth` extension checks
- [x] Commit (Task 6, Step 2)
...
Checklist:
- [x] Write the failing test (Task 7, Step 1) — mocked fetch, **two cases**: success, and degraded-fallback (hub unreachable → null fields, not an error)
- [x] Confirm it fails (`Cannot find module`) (Task 7, Step 2)
- [x] Write `src/pages/api/agentic-status.ts` (Task 7, Step 3) — fails open to `null` fields if the hub is unreachable, never a 500 to the visitor
...
Checklist:
- [x] Write `scripts/verify-hub.sh` (Task 8, Step 1) — post-deploy smoke test, checks Grafana health + status API
- [x] Run it once against nothing running, confirm the expected-fail path (Task 8, Step 2)
- [x] Write `.github/workflows/validate.yml` (Task 8, Step 3) — 7 jobs:
  - [x] `terraform`, `compose`, `status-api-tests` jobs
  - [x] `workflow-lint` (zizmor, blocks on HIGH)
  - [x] `dependency-review` (blocks on HIGH, runs on Dependabot PRs too — no secrets used)
  - [x] `gitleaks` (full history, zero tolerance, excluded only for Dependabot PRs per the documented reason)
  - [x] `sonar` (excluded for Dependabot PRs — `SONAR_TOKEN` isn't passed to them, not a weakened gate)
  - [x] **`actions/checkout`, `dependency-review-action`, `gitleaks-action`, `sonarqube-scan-action` are pinned to SHAs already verified live in `marcobellingeri.dev`** — reuse them, don't retype
  - [x] **`actions/setup-python` and `hashicorp/setup-terraform` have no verified pin in this portfolio yet — look up the current release SHA before running this workflow for real, do not invent one**
- [x] Write `sonar-project.properties` (Task 8, Step 4) — scope is `services/public-status-api` only; Terraform is Checkov's job, not Sonar's
- [x] Write `.gitleaks.toml` (Task 8, Step 5) — start from defaults, no exceptions yet
